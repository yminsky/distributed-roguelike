...
let%expect_test "multi-player state management" =
  ...
  (* Add first player *)
  let result1 = Game_state.add_player state ~player_id:"alice" ~player_name:"Alice" in
  (match result1 with
  | Ok (_state, player) -> 
  ...
  (* Add second player *)
  let result2 = Game_state.add_player state ~player_id:"bob" ~player_name:"Bob" in
  (match result2 with
  | Ok (_state, player) -> 
    printf "Added player: %s at (%d, %d) with sigil '%c'\n" 
      player.name player.position.x player.position.y player.sigil
...
let%expect_test "collision detection" =
  ...
  (* Add two players *)
  let (state, alice) = (match Game_state.add_player state ~player_id:"alice" ~player_name:"Alice" with
    | Ok result -> result | Error _ -> failwith "Failed to add Alice") in
  let (state, bob) = (match Game_state.add_player state ~player_id:"bob" ~player_name:"Bob" with
  ...
  let alice = Game_state.get_player state ~player_id:"alice" |> Option.value_exn in
...
let%expect_test "visual multi-player rendering" =
  ...
  (* Add three players *)
  let (state, _) = (match Game_state.add_player state ~player_id:"alice" ~player_name:"Alice" with
    | Ok result -> result | Error _ -> failwith "Failed to add Alice") in
  let (state, _) = (match Game_state.add_player state ~player_id:"bob" ~player_name:"Bob" with
    | Ok result -> result | Error _ -> failwith "Failed to add Bob") in
  let (state, _) = (match Game_state.add_player state ~player_id:"charlie" ~player_name:"Charlie" with
