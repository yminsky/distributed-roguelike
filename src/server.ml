...
module Server_state = struct
  ...
  let add_state_writer t writer =
    t.state_writers := writer :: !(t.state_writers)

  let remove_state_writer t writer =
    t.state_writers := List.filter !(t.state_writers) ~f:(fun w -> not (phys_equal w writer))

  let broadcast_update t update =
    List.iter !(t.state_writers) ~f:(fun writer ->
      if not (Pipe.is_closed writer) then
        Pipe.write_without_pushback writer update)
...
let handle_request server_state connection_state request =
  ...
  | Move { direction } ->
    (match connection_state.Connection_state.player_id with
     | None ->
       return (Protocol.Response.Error "Not joined")
     | Some player_id ->
       (match Server_state.move_player server_state ~player_id ~direction with
        | Ok () ->
          printf "Player %s moved %s\n%!" player_id (Protocol.Direction.to_string direction);
          return (Protocol.Response.Ok)
        | Error msg ->
...
let handle_state_rpc server_state connection_state connection =
  let your_id = 
    ...
    | None -> "" (* Not joined yet *)
  in
  let initial_state = Protocol.Initial_state.{
    your_id;
...
let create_implementations server_state =
  ...
  in
  let state_impl =
    Rpc.State_rpc.implement (Protocol.Rpc_calls.get_game_state ()) (fun connection_state () ->
      (* We need the actual RPC connection, but we don't have access to it here *)
      (* For now, we'll pass a dummy connection and fix this later *)
